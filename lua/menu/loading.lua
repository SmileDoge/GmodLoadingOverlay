...
function PANEL:Paint()
	...
	if ( self.JavascriptRun && gmodOverlayIsActive() && !gmodOverlayIsLoading() ) then

		gmodOverlayRunJavascript( self.JavascriptRun )
...
function PANEL:RunJavascript( str )
	...
	if ( !gmodOverlayIsActive() ) then return end
	if ( gmodOverlayIsLoading() ) then return end

	gmodOverlayRunJavascript( str )
...
function PANEL:StatusChanged( strStatus )
	...
	if ( matchedFileName ) then
		...
		gmodOverlayRunJavascript( "if ( window.DownloadingFile ) DownloadingFile( '" .. matchedFileName:JavascriptSafe() .. "' )" )
	...
	if ( startPos ) then
		...
		gmodOverlayRunJavascript( "if ( window.DownloadingFile ) DownloadingFile( '" .. fileName:JavascriptSafe() .. "' )" )
	...
	gmodOverlayRunJavascript( "if ( window.SetStatusChanged ) SetStatusChanged( '" .. strStatus:JavascriptSafe() .. "' )" )
...
function PANEL:RefreshDownloadables()
	...
	gmodOverlayRunJavascript( "if ( window.SetFilesNeeded ) SetFilesNeeded( " .. iDownloading .. ")" )
	gmodOverlayRunJavascript( "if ( window.SetFilesTotal ) SetFilesTotal( " .. iFileCount .. ")" )
...
function IsInLoading()
	...
	if ( !IsValid( pnlLoading ) || !gmodOverlayIsActive() ) then
		return false
	end
